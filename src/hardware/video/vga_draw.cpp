...
ImageInfo setup_drawing()
{
	...
	if (vga.draw.ttf.override) {
		...
		case MachineType::Vga:
			if (is_high_resolution_mode(CurMode)) {
				// High-resolution (640x480 or above, 256
				// colours or hi/true color) VESA and SVGA games
				// and demos don't reprogram the VGA registers
				// in the middle of the screen, so we can draw
				// these in chunks which is more performant.
				vga.draw.mode = DrawMode::Part;

			} else {
				// Per-scanline rendering for accurate mid-frame
				// VGA register changes (raster effects, palette
				// changes in games like Lemmings and Pinball
				// Dreams).
				vga.draw.mode = DrawMode::Scanline;
			}
			break;
	...
	if (vga.draw.ttf.override || vga.draw.ttf.keep_checking_rom_font) {

...
// The largest environment COMMAND /e accepts (MS-DOS 5.0 reference,
// COMMAND); the bound when no memory block describes one.
static constexpr uint32_t MaxEnvironmentBytes = 32768;

namespace {

struct EnvironmentEntries {
	std::vector<std::string> entries = {};
	// False when the block ended before the empty entry that closes the
	// list.
	bool terminated = false;
};

// Reads each entry to its NUL, never past size bytes from start.
EnvironmentEntries read_environment(const PhysPt start, const uint32_t size)
{
	EnvironmentEntries result = {};
	uint32_t offset           = 0;
	for (;;) {
		std::string entry = {};
		bool ended        = false;
		while (offset < size) {
			const auto c = mem_readb(start + offset++);
			if (c == 0) {
				ended = true;
				break;
			}
			entry.push_back(static_cast<char>(c));
		}
		if (!ended) {
			return result;
		}
...
std::optional<std::string> DOS_PSP::GetEnvironmentValue(const std::string_view variable) const
{
	...
	const auto env = read_environment(PhysicalMake(GetEnvironment(), 0),
	                                  MaxEnvironmentBytes);
	for (const auto& entry : env.entries) {
		const auto equal = entry.find('=');
		if (equal == variable.size() &&
		    iequals(variable, std::string_view(entry).substr(0, equal))) {
			return entry.substr(equal + 1);
		}
	}
	return {};
}
...
{
	return read_environment(PhysicalMake(GetEnvironment(), 0),
	                        MaxEnvironmentBytes)
	        .entries;
}
...
{
	// A program may free its environment and leave or zero PSP:2Ch; only
	// an owned memory block behind the segment is safe to write.
	constexpr uint8_t MiddleMcbType = 0x4d;
	constexpr uint8_t LastMcbType   = 0x5a;
	if (GetEnvironment() == 0) {
		return false;
	}
	DOS_MCB mcb(GetEnvironment() - 1);
	if ((mcb.GetType() != MiddleMcbType && mcb.GetType() != LastMcbType) ||
	    mcb.GetPSPSeg() == MCB_FREE) {
		return false;
	}

	// Read everything before writing anything, so a refused change leaves
	// the block as it was.
	const PhysPt env_start  = PhysicalMake(GetEnvironment(), 0);
	const uint32_t envsize  = mcb.GetSize() * 16u;
	const auto entry_length = variable.size();

	auto env = read_environment(env_start, envsize);
	if (!env.terminated) {
		return false;
	}
	...
	for (auto& entry : env.entries) {
		...
		if (entry.size() > entry_length && entry[entry_length] == '=' &&
		    iequals(variable, std::string_view(entry).substr(0, entry_length))) {
			continue;
		}
		kept_bytes += entry.size() + 1;
		kept.push_back(std::move(entry));
	}

	// ensure room
	if (envsize <= kept_bytes + variable.size() + 1 + new_string.size() + 2) {
		return false;
	}

	PhysPt env_write = env_start;
	auto write_entry = [&env_write](const std::string& entry) {
		MEM_BlockWrite(env_write, entry.c_str(), entry.size() + 1);
		env_write += static_cast<PhysPt>(entry.size() + 1);
	};
	...
	if (!new_string.empty()) {
		...
		write_entry(bigentry + "=" + std::string(new_string));
	}

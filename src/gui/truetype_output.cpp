...
using RenderDataLine   = std::array<uint8_t, ScalerMaxWidth * BytesPerPixel>;
...
bool TTF_ShouldOverrideScreen()
{
	...
	vga.draw.ttf.keep_checking_rom_font = wants_override;
...
static void check_if_screen_font_is_unaltered()
{
	...
	case 16: {
		...
		is_screen_font_unaltered = check_font(16, vram_font_0, rom_font_16) &&
	...
	case 14: {
		...
		is_screen_font_unaltered = check_font(14, vram_font_0, rom_font_14) &&
		...
		if (!has_extension) {
			...
			for (const auto& directory : directories) {
				const auto result = find_font_file(
				        directory, font_file_name_extension);
				if (!result.empty()) {
					return result;
				}
			}
...
void TTF_ReadConfigFont(SectionProp& section)
{
	...
	if (!custom_font_path.empty() && screen_font.Load(custom_font_path)) {
		...
		augra::log_info("ttf", "Loaded font '%s'", custom_font_path.c_str());
		maybe_recalculate_drawing();
		return;
	}
...
void TTF_AddConfigOptions(SectionProp& section)
{
	...
	auto pstring = section.AddString("ttf_aspect", Always, OptionAspect::WideSticky);
	pstring->SetValues({OptionAspect::Original,
	                    OptionAspect::Wide,
	                    OptionAspect::WideSticky,
	                    OptionAspect::Font});
	pstring->SetHelp(
	        "Selects the TrueType font aspect ratio ('wide' by default):\n"
	        "\n"
